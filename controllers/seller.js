...
module.exports.getSeller = (req,res) => {
  Seller.findAll()
  .then((seller) => {
    res.json({"status": 200, "data" : seller, "pesan" : "Berhasil"});
  })
  .catch((error) => {
...
module.exports.deleteSeller = (req,res) => {
  Seller
      ...
      .then((seller) => {
        ...
        if(seller == 1){
          res.json({"status": 200, "data" : seller, "pesan" : "Berhasil terhapus"});
        }else{
          res.json({"status": 200, "data" : seller, "pesan" : "data tidak ada, kemungkinan telah terhapus"});
        }
      })
      .catch((error) => {
        res.json({"status": 500, "data" : {}, "pesan" : error.toString()});
...
module.exports.updateSeller = (req,res) => {
  ...
  Seller
      ...
      .then((seller) => {
        if(seller[0] == 1){
          res.json({"status": 200, "data" : seller, "pesan" : "update berhasil"});
        }else{
          res.json({"status": 200, "data" : seller, "pesan" : "update tidak dapat dilakukan, data yang di tuju tidak ada"});
        }
      })
      .catch((error) => {
        res.json({"status": 500, "data" : {}, "pesan" : error.toString()});
